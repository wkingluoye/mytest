...

{
	 ...
	 * @throws Exception
	 */
	private final static String TAG = "SocketHttpRequester";
	 ...
	 * 
	 *            因为它会指向手机模拟器，你可以使用http://www.baidu.com或http://192.168.1.10:8080
	 *            这样的路径测试)
	 * 
	 * @param params
...

	{
		...
		// 建立一个Socket链接
		//Log.i(TAG, "|Socket:"+url.getHost()+"|"+port);
		
		Socket socket = new Socket(InetAddress.getByName(url.getHost()), port);
		//Log.i(TAG, "|Socket:"+socket.isConnected());
		...
		c = reader.read();
		while (c != -1){
            ret = String.valueOf((char)c)+reader.readLine();
            
            //System.out.println( String.valueOf((char)c)+"|"+ret);
            c = reader.read();
        }
		
		//uiHandler.sendMessage(uiHandler.obtainMessage(1, str));          
		outStream.flush();
		outStream.close();
		reader.close();
		socket.close();
